...
| `<Variable>` | `{{var}}` or `{{self.*}}` interpolation | `name`, `source`, `resolved-at` (always `compile-time` for v1) | `source-path`, `toml-layer` (required when `source=toml`; one of `defaults`/`team`/`user`/`central-base-team`/`central-base-user`/`central-custom-team`/`central-custom-user`/`merged`), `contributing-paths` (used when `toml-layer=merged`, JSON-encoded list of contributing file paths), `declared-by` (module ID that originally declared the variable), `template-from` (`module.yaml` whose `result:` template was applied), `base-source-path` (the defaults file when the value came from an override layer) |
| `<TomlGlobExpansion>` | Boundary around content inlined from a `file:`-prefixed TOML array entry expanded via filesystem glob | `pattern` (the literal glob pattern, with `{project-root}`-style vars substituted), `source` (`toml`), `toml-layer` (one of the per-skill or central-TOML values — see `<Variable>` row above; `merged` not used here, see Appendix A), `source-path` (the TOML file that contained the pattern), `toml-field` (dotted path into the merged TOML, e.g. `agent.persistent_facts`), `match-count` | — |
...
| `toml` | Value came from the merged TOML layer stack. Per-skill layers (scoped to the compiling skill): `<skill>/customize.toml` defaults + `_bmad/custom/<skill>.toml` team + `_bmad/custom/<skill>.user.toml` user. Central layers (process-global, PR #2285): `_bmad/config.toml` base-team + `_bmad/config.user.toml` base-user + `_bmad/custom/config.toml` custom-team + `_bmad/custom/config.user.toml` custom-user. Per-skill tiers outrank central tiers — see 8-tier cascade + "specific wins over general" note above. The `toml-layer` attribute indicates which layer supplied the value. | `self.*` |
...
- **`self.*` cascade (TOML-sourced, 8 tiers, highest → lowest):**
  1. `install-flag` — CLI flag at the current invocation.
  2. `toml/user` — `_bmad/custom/<skill>.user.toml` (per-skill user layer).
  3. `toml/team` — `_bmad/custom/<skill>.toml` (per-skill team layer).
  4. `toml/defaults` — `<skill>/customize.toml` (per-skill shipped defaults).
...
**Precedence principle: specific scope wins over general scope.** Per-skill TOML layers (tiers 2–4) are lexically scoped to the skill being compiled and outrank the process-global central TOML layers (tiers 5–8) — even when a per-skill layer is a *shipped default* and the central layer is an *explicit user override*. A skill author who declares `agent.icon = "📋"` in PM's `customize.toml` is asserting per-skill intent; a user's central `[agent] icon = "🦊"` expresses a global preference but does not override per-skill intent. To change PM's icon, the user must override per-skill (via `_bmad/custom/bmad-agent-pm.user.toml`, tier 2). The two namespaces (`self.*` vs. non-`self.`) are lexically distinct so names never collide across cascades. A variable's `source` reflects the first tier in which the name appears during its namespace's precedence walk.
...
- For `source: toml` variables, `toml_layer` is required (one of `defaults`/`team`/`user`/`central-base-team`/`central-base-user`/`central-custom-team`/`central-custom-user`/`merged`); `contributing_paths` is required when `toml_layer=merged`; `source_path` is omitted when `toml_layer=merged`.
...
- FR23: Drift triage in v1 happens through the `bmad-customize` skill (FR56), not via a standalone CLI subcommand. Prose fragment drift is triaged via a three-way merge UX (upstream-old → upstream-new → user-override) the skill walks the user through in chat. TOML semantic drift is triaged via a field-level review UX — same skill, same session, different per-entry presentation. Glob-input drift is reported informationally unless intersecting a user-overridden TOML field. There is no `bmad upgrade --reconcile` flag in v1; the skill IS the triage UX.
- FR24: All install/upgrade subcommands accept `--directory`, `--modules`, `--tools`, `--override-root`, `--yes`, and `--debug` flags. `--lock-timeout-seconds <N>` is an advanced flag (default: 300s) tuning the advisory-lock timeout for the lazy-compile guard and batch-mode lockfile writes; documented as advanced since most users never need it but CI operators running slow compile environments may.
...
- **NFR-C2 · OS matrix.** Officially supported: macOS (Intel + Apple Silicon), Linux (x86_64 + ARM64), Windows 10/11. CI schedule: Linux (x86_64) runs on every PR; all six OS/arch combinations run on merge-to-`main`, nightly builds, and release-tag builds. Rationale: the `bmad_compile/io.py` determinism boundary (NFR-R1) contains cross-OS divergence to a single module, so per-PR three-OS coverage is insurance the design already guarantees. Nightly detection of any regression lands within a day of the offending merge. Trade: up to one-day blind to macOS/Windows regressions. Accepted.
...
- **NFR-M2 · Test coverage for resolution tiers.** The compile engine has unit tests covering every adjacent pair of fragment resolution tiers (base↔variant, variant↔user-override, user-override↔user-module-fragment, user-module-fragment↔user-full-skill), every adjacent pair of variable resolution tiers in both namespaces (`self.*` 8-tier cascade: `install-flag↔toml/user`, `toml/user↔toml/team`, `toml/team↔toml/defaults`, `toml/defaults↔toml/central-custom-user`, `toml/central-custom-user↔toml/central-custom-team`, `toml/central-custom-team↔toml/central-base-user`, `toml/central-base-user↔toml/central-base-team`; non-`self.` cascade: `install-flag↔user-config`, `user-config↔module-config`, `module-config↔bmad-config`, `bmad-config↔derived`), and a matrix test exercising all tiers in combination. The per-skill↔central boundary (tier 4↔tier 5) carries a dedicated "specific wins over general" assertion test. TOML merge semantics have their own test matrix against upstream's documented structural rules. Glob-input drift detection has contract tests for add/remove/modify cases.
...
- **NFR-M5 · Error-message vocabulary is stable.** Specific error types (`UNKNOWN_DIRECTIVE`, `UNRESOLVED_VARIABLE`, `MISSING_FRAGMENT`, `CYCLIC_INCLUDE`, `OVERRIDE_OUTSIDE_ROOT`, `LOCKFILE_VERSION_MISMATCH`, `PRECEDENCE_UNDEFINED`) are part of the public contract and do not change names or semantics within v1. `PRECEDENCE_UNDEFINED` is raised by the compiler when it encounters a cross-plane customization interaction (TOML field × prose fragment × YAML variable) not covered by the architecture document's Cross-Plane Customization Precedence Matrix — see architecture §Implementation Patterns for the matrix; the error message points at it as the docs anchor.
...
| `toml-layer` | conditional | `defaults` \| `team` \| `user` \| `central-base-team` \| `central-base-user` \| `central-custom-team` \| `central-custom-user` \| `merged` | Which TOML layer produced the value. Required iff `source=toml`; absent otherwise. Per-skill layers (`defaults`/`team`/`user`) are scoped to the compiling skill; central layers (`central-base-*`/`central-custom-*`) are process-global and sourced from PR #2285's four-file central TOML merged by upstream's `resolve_config.py` order. `merged` indicates structural merge (array append or deep table merge) across multiple layers; in that case `source-path` is omitted and `contributing-paths` is used. |
...
| `toml-layer` | yes | `defaults` \| `team` \| `user` \| `central-base-team` \| `central-base-user` \| `central-custom-team` \| `central-custom-user` | Which TOML layer's array entry produced the pattern. `merged` is not used here — if multiple layers each contribute `file:` entries, they are emitted as multiple `<TomlGlobExpansion>` tags, one per contributing layer. Central TOML layers are permitted sources for `file:` entries (same `{project-root}` containment rules as per-skill globs). |

---
title: "项目上下文"
description: AGENTS.md 如何使用项目规则和偏好指导 AI 智能体
sidebar:
  order: 11
---

`AGENTS.md` 是面向 AI 智能体的项目级上下文文件。它的定位不是教程步骤，而是“实现约束说明”：把你的技术偏好、架构边界和工程约定沉淀成可复用规则，让不同工作流、不同智能体在多个 `story` 中做出一致决策。
...
有 `AGENTS.md` 时，这些高频偏差会明显减少，因为关键规则在进入实现前已经被显式声明。

## 它如何被工作流使用

多数实现相关工作流会自动加载 `AGENTS.md`（若存在），并把它作为共享上下文参与决策。
...
| **新项目（架构后）** | 通过 `bmad-project-context` 生成并补充 | 把架构决策转成可执行规则 |
...
默认位置是 `AGENTS.md`。工作流优先在该位置查找，也会扫描项目内的 `**/AGENTS.md`。

...
description: Cách AGENTS.md định hướng các agent AI theo quy tắc và ưu tiên của dự án
sidebar:
  order: 11
---

Tệp `AGENTS.md` là kim chỉ nam cho việc triển khai của các agent AI trong dự án của bạn. Tương tự như một "bản hiến pháp" trong các hệ thống phát triển khác, nó ghi lại các quy tắc, pattern và ưu tiên giúp việc sinh mã được nhất quán trong mọi workflow.
...
Tệp `AGENTS.md` giải quyết vấn đề này bằng cách tài liệu hóa những gì agent cần biết trong định dạng ngắn gọn, tối ưu cho LLM.

## Nó hoạt động như thế nào

Mỗi workflow triển khai đều tự động nạp `AGENTS.md` nếu tệp tồn tại. Workflow architect cũng nạp tệp này để tôn trọng các ưu tiên kỹ thuật của bạn khi thiết kế kiến trúc.
...
- `bmad-build` - áp dụng pattern khi triển khai các spec
- `bmad-sprint-planning`, `bmad-retrospective`, `bmad-correct-course` - cung cấp bối cảnh cấp dự án

## Khi nào nên tạo

Tệp `AGENTS.md` hữu ích ở bất kỳ giai đoạn nào của dự án:
...
| **Dự án mới, sau kiến trúc** | Qua `bmad-project-context` hoặc tạo thủ công | Ghi lại quyết định kiến trúc cho các agent triển khai |
| **Dự án hiện có** | Qua `bmad-project-context` | Khám phá pattern hiện có để agent theo đúng quy ước |
| **Dự án Quick Flow** | Trước hoặc trong `bmad-build` | Đảm bảo triển khai nhanh vẫn tôn trọng pattern của bạn |
...
Tạo tệp tại `AGENTS.md` và thêm các quy tắc của bạn:
...
Chạy workflow `bmad-project-context` sau khi bạn hoàn tất kiến trúc:
...
Với dự án hiện có, chạy `bmad-project-context` để khám phá pattern sẵn có:
...
Nếu không có `AGENTS.md`, các agent sẽ tự đưa ra giả định có thể không phù hợp với dự án:
...
Tệp `AGENTS.md` là tài liệu sống. Hãy cập nhật khi:
...
Bạn có thể sửa thủ công bất kỳ lúc nào, hoặc chạy lại `bmad-project-context` để cập nhật sau các thay đổi lớn.

:::note[Vị trí tệp]
Vị trí mặc định là `AGENTS.md`. Các workflow tìm tệp ở đó, đồng thời cũng kiểm tra `**/AGENTS.md` ở bất kỳ đâu trong dự án.
:::

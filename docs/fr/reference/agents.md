...
| Développeur (Amelia)        | `bmad-agent-dev`         | `DS`, `QD`, `QA`, `CR`, `SP`, `CS`, `ER` | Dev Story, Build, Génération de Tests QA, Code Review, Sprint Planning, Créer Story, Rétrospective d’Epic |
...
Exemples : `PRD` (Créer, mettre à jour ou valider un PRD), `DS` (Dev Story), `CA` (Créer l’architecture), `QD` (Build)

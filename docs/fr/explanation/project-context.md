...
description: Comment AGENTS.md guide les agents IA avec les règles et préférences de votre projet
sidebar:
  order: 11
---

Le fichier `AGENTS.md` est le guide d’implémentation de votre projet pour les agents IA. Similaire à une « constitution » dans d’autres systèmes de développement, il capture les règles, les patterns et les préférences qui garantissent une génération de code cohérente à travers tous les workflows.
...
Le fichier `AGENTS.md` résout ce problème en documentant ce que les agents doivent savoir dans un format concis et optimisé pour les LLM.

## Comment Ça Fonctionne

Chaque workflow d’implémentation charge automatiquement `AGENTS.md` s’il existe. Le workflow architecte le charge également pour respecter vos préférences techniques lors de la conception de l’architecture.
...
Le fichier `AGENTS.md` est utile à n’importe quel stade d’un projet :
...
| **Nouveau projet, après l’architecture** | Via `bmad-project-context` ou manuellement | Capturer les décisions d’architecture pour les agents d’implémentation                |
| **Projet existant**                      | Via `bmad-project-context`                 | Découvrir les patterns existants pour que les agents suivent les conventions établies |
...
Créez le fichier `AGENTS.md` et ajoutez vos règles :
...
Exécutez le workflow `bmad-project-context` après avoir terminé votre architecture :
...
Pour les projets existants, exécutez `bmad-project-context` pour découvrir les patterns existants :
...
Sans `AGENTS.md`, les agents font des suppositions qui peuvent ne pas correspondre à votre projet :
...
Le fichier `AGENTS.md` est un document vivant. Mettez-le à jour quand :
...
Vous pouvez l’éditer manuellement à tout moment, ou réexécuter `bmad-project-context` pour le mettre à jour après des changements significatifs.

:::note[Emplacement du Fichier]
L’emplacement par défaut est `AGENTS.md`. Les workflows le recherchent là, et vérifient également `**/AGENTS.md` n’importe où dans votre projet.
:::

...
2. **Otherwise:**
   ...
   - If `{spec_file}` is unknown or missing, create `{implementation_artifacts}/bmad-build-auto-result-<slug-or-timestamp>.md` with:
     ...
     Blocking condition: <blocking condition, if any>
     ```
3. Run: `python3 {project-root}/_bmad/scripts/resolve_customization.py --skill {skill-root} --key workflow.on_complete` — if the resolved value is non-empty, follow it as the final terminal instruction before exiting.
4. Stop the workflow.

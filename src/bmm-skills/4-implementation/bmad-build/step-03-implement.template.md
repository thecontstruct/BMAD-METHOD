...
<<include path="_shared/fragments/sub-agent-activation.template.md" spawn_action="Hand `{spec_file}` to a subagent/task and let it implement. If the platform allows, keep the subagent available for re-engagement after it returns — step-04 may send it review fixes." fallback_action="implement directly.">>

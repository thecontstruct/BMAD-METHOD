...
2. <<include path="_shared/fragments/sub-agent-activation.template.md" spawn_action="Launch Blind Hunter, Edge Case Hunter, and Verification Gap Reviewer in parallel without prior conversation context. If `{review_mode}` = `&#34;full&#34;`, include the Acceptance Auditor in the same parallel launch." fallback_action="generate prompt files in `{implementation_artifacts}` for each applicable reviewer role and HALT. Ask the user to run each in a separate session (ideally a different LLM) and paste back the findings. When findings are pasted, resume from this point and proceed to step 3.">>

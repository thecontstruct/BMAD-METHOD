...
<<include path="_shared/fragments/sub-agent-activation.template.md" spawn_action="Launch Blind Hunter without prior conversation context." fallback_action="generate one review prompt file in `{implementation_artifacts}` and HALT. Ask the human to run it in a separate session and paste back the findings.">>

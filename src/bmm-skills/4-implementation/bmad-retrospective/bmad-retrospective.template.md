...
<action>For each story in epic {epic_number}, read the complete story record from <ArtifactPath kind="story" epic="{epic_number}" story="{story_num}" /> (legacy story files) and {implementation_artifacts}/spec-{epic_number}-{story_num}-*.md (build specs).</action>
